...
program
  ...
  .action(async () => {
    try {
      ...
      const host = getHost();
      if (host === null) throw new Error("Not authenticated");
      const enet = new ENet(host, token);
      await enet.reboot();
  ...
  .action(async (command, project) => {
    try {
      switch (command) {
        case "ls":
          await (async () => {
            const response = await new ENetClient(getHost(), getISID()).getProjectUIDs();
            if (json) {
              if (pretty) {
                console.log(util.inspect(response, false, null, true));
              } else {
        ...
        case "inspect":
          await (async () => {
            ...
            const response = await new ENetClient(getHost(), getISID()).getProjectInformation(project);
            if (json) {
              if (pretty) {
                console.log(util.inspect(response, false, null, true));
              } else {
...
program
  ...
  .action(async (fuid, command, artifact, value) => {
    try {
      const client = new ENetClient(getHost(), getISID());
      if (fuid === "ls") {
            ...
            for (const { outputDeviceFunctions } of deviceChannels) {
              for (const { currentValues } of outputDeviceFunctions) {
                ...
                for (const { valueTypeID, value } of values) {
                  console.log(`  ${valueTypeID} ${value ? chalk.greenBright("ON") : chalk.redBright("OFF")}`);
                }
              }
            }
          }
  ...
  .action(async (fuid, action) => {
    try {
      switch (action) {
        ...
        case "disable":
          await (async () => {
            await new ENetClient(getHost(), getISID()).callInputDeviceFunction(fuid, action === "enable");
...
function getISID(): string {
  try {
    return JSON.parse(fs.readFileSync(cpath, 'utf8')).s;
  } catch (error) {
    return null;
  }
}
...
function getHost(): string {
  try {
    return JSON.parse(fs.readFileSync(cpath, 'utf8')).h;
  } catch (error) {
    return null;
  }
}
...
function getUser(): string {
  try {
    return JSON.parse(fs.readFileSync(cpath, 'utf8')).u;
  } catch (error) {
    return null;
  }
}
